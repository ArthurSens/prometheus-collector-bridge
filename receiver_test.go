...
// TestEndToEnd_FullReceiverPipeline exercises the full flow:
// Factory → Receiver creation → Start (with real Prometheus metrics) → Scrape → Consumer receives metrics → Shutdown.
...
// TestEndToEnd_MultipleScrapes verifies that the receiver continues scraping
// and the consumer accumulates metrics over time.
